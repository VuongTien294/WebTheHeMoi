...
import '../css/magnific-popup.css'
 ...
 class CardSearchByName extends React.Component {

    constructor(props) {
        super(props)

        let { match } = this.props
        let { productName } = match.params
    ...
    loadProductByName = async () => {
        if (this.state.productName != null || this.state.productName != "") {
            ...
            try {
                let response = await fetch("http://localhost:5000/product/getbyname/" + this.state.productName.toUpperCase(), requestOptions)
                if (response.ok) {
...
export default withRouter(CardSearchByName)